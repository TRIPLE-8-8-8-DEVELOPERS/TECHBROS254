    ...
    <section ref={sectionRef} className="py-20 bg-white">
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-5xl mx-auto">
          <div 
            className={`flex flex-wrap justify-center mb-8 border-b transition-all duration-700 delay-200 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
            }`}
          >
            {technologies.map((category, index) => (
              ...
              >
                {category.name}
                {activeCategory === index && (
                  <span className="absolute h-0.5 bg-tech-400 bottom-0 left-0 right-0 rounded-t-lg" />
                )}
              </button>
            ))}
          </div>
          ...
          >
            {technologies[activeCategory].items.map((tech, index) => (
              <div 
                key={index} 
                className="bg-gray-50 p-4 rounded-lg text-center border border-gray-100 hover:border-tech-200 transition-all duration-300 hover:shadow-sm"
              >
