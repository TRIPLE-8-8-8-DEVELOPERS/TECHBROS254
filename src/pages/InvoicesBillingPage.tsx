...
import { FileText, CreditCard, Bitcoin, Download, CheckCircle, XCircle, Clock, Plus, Edit, AlertCircle, Home, Folder, MessageCircle, Calendar, BarChart2, File as FileIcon, Settings, LogOut, HelpCircle } from "lucide-react";
...
const navItems = [
  ...
  { icon: <FileIcon />, label: "Documents & Files", path: "/documents" },
          ...
          <SidebarMenu>
            {navItems.map((item) => (
              <SidebarMenuItem key={item.label}>
                <SidebarMenuButton isActive={item.label === "Invoices & Billing"} onClick={() => window.location.pathname !== item.path && window.location.assign(item.path)}>
                  {item.icon}
            ...
            <SidebarSeparator />
            <SidebarMenuItem>
              <SidebarMenuButton onClick={() => window.location.assign('/auth')}>
                <LogOut />
                <span>Logout</span>
              </SidebarMenuButton>
            </SidebarMenuItem>
                    ...
                    <div className="flex flex-col gap-2">
                      {paymentMethods.map((pm, i) => (
                        <div key={i} className="flex items-center gap-3 bg-gray-50 dark:bg-[#23272F]/60 rounded-lg px-4 py-2">
                          ...
                          {pm.type === 'PayPal' && <CreditCard size={18} className="text-[#1E90FF]" />}
